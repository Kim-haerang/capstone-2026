...
def setup_gpio() -> None:
    global GPIO_READY, GPIO_ERROR, SERVO_FACTORY, SERVO
    if GPIO_READY:
        return
    if GPIO is None:
...
def run_pump(detergent_key: str, pump_ms: int) -> None:
    setup_gpio()
    if detergent_key not in PUMP_PINS:
        raise RuntimeError(f"unknown_detergent_key:{detergent_key}")
    pin = PUMP_PINS[detergent_key]
    all_pumps_off()
    GPIO.output(pin, _pump_on_state())
    try:
        time.sleep(max(0, pump_ms) / 1000.0)
    finally:
        GPIO.output(pin, _pump_off_state())


def _brush_profile(brush_intensity: str) -> Tuple[List[int], float]:
    if brush_intensity == "high":
        return [SERVO_MIN_ANGLE, SERVO_HIGH_ANGLE], 0.18
    if brush_intensity == "medium":
        return [SERVO_MIN_ANGLE, SERVO_MEDIUM_ANGLE], 0.22
    return [SERVO_MIN_ANGLE, SERVO_LOW_ANGLE], 0.28
...
def run_brush(brush_ms: int, brush_intensity: str) -> None:
    ...
    angles, step_delay = _brush_profile(brush_intensity)
    end_time = time.time() + max(0, brush_ms) / 1000.0
    try:
...
def build_treatment(source: str, material_group: str) -> Treatment:
    ...
    pump_pin = PUMP_PINS[detergent_key]
    detergent_ko = DETERGENT_LABEL_KO.get(detergent_key, detergent_key)
    intensity_ko = INTENSITY_LABEL_KO.get(intensity, intensity)
    summary = (
        f"{detergent_ko} 기준으로 GPIO {pump_pin} 펌프를 {pump_ms}ms 분사하고, "
        f"브러시는 GPIO {SERVO_PIN}에서 {intensity_ko} 강도로 {brush_ms}ms 작동합니다. "
...
def run_motor(detergent_key: str, pump_ms: int, brush_ms: int, brush_intensity: str) -> None:
    run_pump(detergent_key, pump_ms)
    run_brush(brush_ms, brush_intensity)
...
def execute(payload: ExecuteRequest):
    try:
        ...
        run_motor(detergent_key, payload.pump_ms, payload.brush_ms, payload.brush_intensity)
        return {
            "ok": True,
