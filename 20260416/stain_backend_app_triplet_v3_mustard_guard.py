...
PUMP_PINS = {
    "water_based_detergent": int(os.environ.get("PUMP_WATER_PIN", "17")),
    "mixed_detergent": int(os.environ.get("PUMP_MIXED_PIN", "27")),
    "oil_based_detergent": int(os.environ.get("PUMP_OIL_PIN", "22")),
    "rinse_water": int(os.environ.get("PUMP_RINSE_PIN", "23")),
}
...
ARDUINO_PUMP_MAP = {
    "water_based_detergent": 1,
    "mixed_detergent": 2,
    "oil_based_detergent": 3,
    "rinse_water": 4,
}
PUMP_ACTIVE_STATE = os.environ.get("PUMP_ACTIVE_STATE", "HIGH").upper()
SERVO_MIN_ANGLE = int(os.environ.get("SERVO_MIN_ANGLE", "0"))
SERVO_LOW_ANGLE = int(os.environ.get("SERVO_LOW_ANGLE", "10"))
SERVO_MEDIUM_ANGLE = int(os.environ.get("SERVO_MEDIUM_ANGLE", "20"))
SERVO_HIGH_ANGLE = int(os.environ.get("SERVO_HIGH_ANGLE", "30"))
SERVO_SETTLE_SEC = float(os.environ.get("SERVO_SETTLE_SEC", "0.12"))
SERVO_CENTER_ANGLE = int(os.environ.get("SERVO_CENTER_ANGLE", "90"))
SERVO_SWING_ANGLE = int(os.environ.get("SERVO_SWING_ANGLE", "30"))
SERVO_STEP_DELAY = float(os.environ.get("SERVO_STEP_DELAY", "0.12"))
BRUSH_COUNT_BY_GROUP = {"GROUP_A": 8, "GROUP_B": 6, "GROUP_C": 4}
PRE_WATER_MS = int(os.environ.get("PRE_WATER_MS", "800"))
POST_RINSE_MS = int(os.environ.get("POST_RINSE_MS", "800"))
...
def _pump_on_state() -> int:
    if GPIO is None:
        raise RuntimeError("rpi_gpio_import_failed")
    return GPIO.HIGH if PUMP_ACTIVE_STATE == "LOW" else GPIO.LOW


def _pump_off_state() -> int:
    if GPIO is None:
        raise RuntimeError("rpi_gpio_import_failed")
    return GPIO.LOW if PUMP_ACTIVE_STATE == "LOW" else GPIO.HIGH
...
def setup_gpio() -> None:
    ...
    if GPIO_READY:
        return
    if GPIO is None:
        GPIO_ERROR = "RPi.GPIO import failed. Run on Raspberry Pi with RPi.GPIO installed."
    ...
    try:
        ...
        for pin in PUMP_PINS.values():
            ...
            GPIO.output(pin, _pump_off_state())
        GPIO_READY = True
        GPIO_ERROR = None
...
def _init_servo() -> None:
    global SERVO_FACTORY, SERVO, GPIO_ERROR
    if SERVO is not None:
        return
    if AngularServo is None or LGPIOFactory is None:
        GPIO_ERROR = "gpiozero AngularServo/LGPIOFactory import failed. Install gpiozero and lgpio."
        raise RuntimeError(GPIO_ERROR)
    try:
        SERVO_FACTORY = LGPIOFactory()
        SERVO = AngularServo(
            SERVO_PIN,
...
def _brush_profile(brush_intensity: str) -> Tuple[List[int], float]:
    return [SERVO_CENTER_ANGLE - SERVO_SWING_ANGLE, SERVO_CENTER_ANGLE + SERVO_SWING_ANGLE], SERVO_STEP_DELAY


def _servo_move_and_settle(angle: int) -> None:
    if SERVO is None:
        raise RuntimeError("servo_not_initialized")
    SERVO.angle = max(0, min(180, int(angle)))
    time.sleep(SERVO_STEP_DELAY)


def run_brush_count(brush_count: int) -> None:
    setup_gpio()
    _init_servo()
    if SERVO is None:
        raise RuntimeError("servo_not_initialized")
    center = max(0, min(180, SERVO_CENTER_ANGLE))
    left = max(0, min(180, SERVO_CENTER_ANGLE - SERVO_SWING_ANGLE))
    right = max(0, min(180, SERVO_CENTER_ANGLE + SERVO_SWING_ANGLE))
    try:
        _servo_move_and_settle(center)
        for _ in range(max(0, int(brush_count))):
            _servo_move_and_settle(left)
            _servo_move_and_settle(right)
        _servo_move_and_settle(center)
...
def run_brush(brush_ms: int, brush_intensity: str) -> None:
    count = 6 if brush_intensity == "medium" else (8 if brush_intensity == "high" else 4)
    run_brush_count(count)
...
def build_treatment(source: str, material_group: str) -> Treatment:
    ...
    pump_pin = ARDUINO_PUMP_MAP[detergent_key]
    detergent_ko = DETERGENT_LABEL_KO.get(detergent_key, detergent_key)
    intensity_ko = INTENSITY_LABEL_KO.get(intensity, intensity)
    summary = (
        f"물 {PRE_WATER_MS}ms → {detergent_ko} {pump_ms}ms → 브러시 {brush_count}회 왕복 → 물 {POST_RINSE_MS}ms 순서로 작동합니다. "
        f"브러시는 GPIO {SERVO_PIN}에서 중심 {SERVO_CENTER_ANGLE}도 기준 좌우 {SERVO_SWING_ANGLE}도씩 움직이며, 표기 강도는 {intensity_ko}입니다. "
...
def run_motor(detergent_key: str, pump_ms: int, brush_count: int, brush_intensity: str) -> None:
    run_pump("rinse_water", PRE_WATER_MS)
    run_pump(detergent_key, pump_ms)
    run_brush_count(brush_count)
    run_pump("rinse_water", POST_RINSE_MS)
...
def analyze_triplet(payload: AnalyzeTripletRequest):
    ...
    try:
        ...
        LAST_EXECUTION_PLAN.update({
            ...
            "pump_pin": treatment.pump_pin,
            "pump_ms": treatment.pump_ms,
            "brush_ms": treatment.brush_ms,
...
def test_motor():
    try:
        run_motor("mixed_detergent", 500, BRUSH_COUNT_BY_GROUP.get("GROUP_A", 8), "medium")
        return {
            "ok": True,
            "sequence": [
                {"detergent_key": "rinse_water", "pump_ms": PRE_WATER_MS},
                {"detergent_key": "mixed_detergent", "pump_ms": 500},
                {"brush_count": BRUSH_COUNT_BY_GROUP.get("GROUP_A", 8)},
                {"detergent_key": "rinse_water", "pump_ms": POST_RINSE_MS},
            ],
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"test_motor_error:{repr(e)}")
...
def execute(payload: ExecuteRequest):
    try:
        ...
        return {
            ...
            "sequence": [
                {"detergent_key": "rinse_water", "pump_ms": PRE_WATER_MS},
                {"detergent_key": detergent_key, "pump_ms": pump_ms},
                {"brush_count": brush_count, "brush_intensity": brush_intensity},
                {"detergent_key": "rinse_water", "pump_ms": POST_RINSE_MS},
            ],
            "pump_ms": pump_ms,
            "brush_count": brush_count,
            ...
            "detergent_key": detergent_key,
            "pump_pin": PUMP_PINS.get(detergent_key),
            "servo_pin": SERVO_PIN,
        }
