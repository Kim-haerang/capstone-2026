...
SERVO_STEP_DELAY = {
    "high": float(os.environ.get("SERVO_STEP_DELAY_HIGH", "0.14")),
    "medium": float(os.environ.get("SERVO_STEP_DELAY_MEDIUM", "0.18")),
    "low": float(os.environ.get("SERVO_STEP_DELAY_LOW", "0.24")),
}
...
def _servo_move_and_settle(angle: int, delay_sec: float) -> None:
    if SERVO is None:
...
def run_brush_servo(brush_count: int, brush_intensity: str = "medium") -> None:
    _init_servo()
    if SERVO is None:
        raise RuntimeError("servo_not_initialized")
    brush_count = max(1, int(brush_count))
    step_delay = max(0.05, float(SERVO_STEP_DELAY.get(brush_intensity, SERVO_STEP_DELAY["medium"])))
    center = max(0, min(180, int(SERVO_CENTER_ANGLE)))
    left = max(0, min(180, center - int(SERVO_SWING_ANGLE)))
    right = max(0, min(180, center + int(SERVO_SWING_ANGLE)))
    try:
        _servo_move_and_settle(center, step_delay)
        for _ in range(brush_count):
            _servo_move_and_settle(left, step_delay)
            _servo_move_and_settle(right, step_delay)
            _servo_move_and_settle(center, step_delay)
...
def build_treatment(source: str, material_group: str) -> Treatment:
    ...
    summary = (
        ...
        f"브러시는 라즈베리 서보(GPIO {SERVO_PIN})로 중심 {SERVO_CENTER_ANGLE}도 기준 좌우 {SERVO_SWING_ANGLE}도씩 움직입니다. "
...
def run_motor(detergent_key: str, pump_ms: int, brush_count: int, brush_intensity: str) -> None:
    ...
    run_brush_servo(brush_count, brush_intensity)
...
def serve_ui():
    ui_path = BASE_DIR / "stain_controller_ui_triplet_feedback_roi_v2 (2).html"
...
def health_check():
    ...
    return {
        ...
        "servo_step_delay": SERVO_STEP_DELAY,
    ...
    try:
        ...
        if use_openai:
            ...
            if openai_data:
                ...
                decision_source = "openai_mustard_brown_recheck" if mustard_brown_recheck else "openai_final"
                treatment = build_treatment(final_label, material_group)
                with _plan_lock:
                    LAST_EXECUTION_PLAN.clear()
                    LAST_EXECUTION_PLAN.update({
                ...
                decision_source = "cnn_fallback_after_mustard_recheck_skip" if mustard_brown_recheck else "cnn_fallback_after_openai_skip"
...
def test_motor():
    try:
        run_motor("mixed_detergent", PUMP_MS_BY_SOURCE["mixed_detergent"], BRUSH_COUNT_BY_GROUP.get("GROUP_A", 8), "medium")
        return {"ok": True}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"test_motor_error:{repr(e)}")


@app.post("/api/underwater", response_model=UnderwaterMotorResponse)
def api_underwater(payload: UnderwaterMotorRequest):
    try:
        run_underwater_motor(payload.duration_ms)
        return UnderwaterMotorResponse(ok=True, duration_ms=payload.duration_ms,
                                       message=f"수중모터 {payload.duration_ms}ms 실행 완료")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"underwater_motor_error:{repr(e)}")


@app.post("/api/underwater/stop")
def api_underwater_stop():
    try:
        arduino_command("UM_OFF", wait_done=False)
        return {"ok": True, "message": "수중모터 정지"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"underwater_stop_error:{repr(e)}")
...
def execute(payload: ExecuteRequest):
    try:
        ...
        step_delay = SERVO_STEP_DELAY.get(brush_intensity, SERVO_STEP_DELAY["medium"])
        estimated_ms = int((brush_count * step_delay * 3 + SERVO_SETTLE_SEC) * 1000)
        return {
            "ok": True,
            "sequence": [
                ...
                {"step": "brush", "brush_count": brush_count, "brush_intensity": brush_intensity,
                 "servo_swing_angle": SERVO_SWING_ANGLE, "servo_step_delay_sec": step_delay, "brush_estimated_ms": estimated_ms, "servo_pin": SERVO_PIN},
            ],
            "pump_ms": pump_ms,
            "brush_count": brush_count,
